...
**AI-powered city logistics orchestration platform** — a digital twin that simulates real-world urban delivery networks on OpenStreetMap roads, consolidates orders through micro-hubs, and reduces congestion, emissions, and delivery time.
...
### 1. Hub Selection Agent
```
Input: Order centroid, required slots, available hubs
Output: Best hub (weighted score)

Scoring Model:
┌────────────────────────────────────────────────┐
│  score = w₁·distance + w₂·capacity +           │
│          w₃·congestion + w₄·accessibility +    │
│          w₅·utilization + w₆·operating_hours   │
│                                                │
│  Weights: 0.25, 0.20, 0.20, 0.15, 0.10, 0.10 │
└────────────────────────────────────────────────┘
```

### 2. Delivery Wave Agent (DBSCAN)
```
...
|------|-------|-------------|------|----------|
| **Hyderabad / Secunderabad** | 3 (Secunderabad, Kukatpally, Miyapur) | OSM roads | 21 | 40 |
| **Visakhapatnam** | 4 (Dwaraka Nagar, Kancharapalem, Gajuwaka, MVP Colony) | OSM roads | 15 | 40 |
...
│   │   │   ├── simulation.py        # Start/stop/reset/scenarios
│   │   │   ├── dashboard.py         # KPIs, map data, recommendations
│   │   │   ├── analytics.py         # Impact metrics, order stats
│   │   │   ├── agents.py            # AI decision history
│   │   │   ├── orders.py            # Order CRUD + optimize
│   │   │   ├── hubs.py              # Hub management
│   │   │   ├── fleet.py             # Vehicle/courier listing
│   │   │   ├── curb.py              # Curb zone reservations
│   │   │   ├── dispatch.py          # Chrome extension dispatch
│   │   │   └── ...
...
- Python 3.11+
- Node.js 18+
- 2GB RAM (for road graph loading)
...
The simulation auto-starts with **Festival Sale** scenario at 3x speed.
...
Open **http://localhost:5173** — you'll see the live simulation dashboard.
...
| CO₂ per delivery | 320g | 145g | **55% less** |
...
1. **Explainable AI** — Every decision shows reasoning, inputs, and confidence score. No black boxes.
...
| Stakeholder | Benefit |
|-------------|---------|
| **Municipal Corporations** | Congestion data, curb analytics, emission tracking for city planning |
